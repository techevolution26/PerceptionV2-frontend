...
export default function NewPerceptionForm({
  topics,
  onSuccess,
  token: tokenProp,
}: NewPerceptionFormProps) {
  const router = useRouter(); // FIXED: Initialized router instances
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    //FIXED: Pre-submission token validation guard to block raw alert popups
    if (!token) {
      setLoading(false);
      onSuccess(null as any); // Safely triggers parent container modal closure
      router.push("/login");
      return;
    }
    ...
    // FIXED: Catch token expirations seamlessly without native alert text dumps
    if (res.status === 401) {
      setLoading(false);
      localStorage.removeItem("token"); // Purging dead session indicators
  ...
  const handleFileChange =
    (kind: MediaKind) => (e: ChangeEvent<HTMLInputElement>) => {
